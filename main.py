...
class DemoDialog(QDialog):
    ...
    def about(self):
        ...
        QMessageBox.about(self, 'About Audio M3U',
                text.decode('utf-8'))
    ...
    def update_metadata(self):
        ...
        for book_id in m3u_ids:
            ...
            #print("Found ID: ",book_id, " Formats: ", db.formats(book_id), " Keys: ", mi.all_field_keys())
            #print("Size is: ", db.field_for("size",book_id))
            #mymap = {book_id : 123000000}
            #db.set_field("size",mymap)
            #print("Size is now: ", db.field_for("size",book_id))
            ...
            #mi.set("title","Big Title")


            cover = get_cover(path)
            #id_cover_map = {book_id : None}
            id_cover_map = ("jpeg", cover)
            #print(cover)
            print(type(cover))
            #mi.set_cover(cover)
            #print(id_cover_map)
#            for b_id, data in iteritems(id_cover_map):
#                print(f"{b_id} : {data}")
            #self.db.set_cover(id_cover_map)
            mi.set("cover_data", id_cover_map)
